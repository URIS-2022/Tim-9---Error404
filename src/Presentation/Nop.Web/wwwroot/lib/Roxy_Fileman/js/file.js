...
function File(filePath, fileSize, modTime, w, h){
  ...
  this.height = (h? h: 0);
  this.Show = function(){
    var html = '<li data-path="'+this.fullPath+'" data-time="'+this.time+'" data-icon="'+this.icon+'" data-w="'+this.width+'" data-h="'+this.height+'" data-size="'+this.size+'" data-icon-big="'+(this.IsImage()?this.fullPath:this.bigIcon)+'" title="'+this.name+'">'; //promenjeno
  ...
  this.Move = function(newPath){
    ...
    var newFullPath = RoxyUtils.MakePath(newPath, this.name); //promenjeno
